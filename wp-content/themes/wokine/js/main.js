...
if (whatsapp) {
	var whatsappToggle = whatsapp.querySelector('.Whatsapp__toggle');
	var whatsappPopup = whatsapp.querySelector('.Whatsapp__popup');
}
...
$(document).ready(function(){
	...
	initHeaderFooter = function(){
		...
		$('.navbar-toggle').on('click', function(e){
			...
			if(whatsapp && whatsappPopup.classList.contains('is--open')) whatsappPopup.classList.remove('is--open');
		...
		if ($browser.mobile) {
			...
			/* var whatsappLink = whatsappPopup.querySelector('a');
			var whatsappURL = whatsappLink.getAttribute('href');
			document.getElementById('WhatsappMessage').addEventListener('input', function(e) {
				var message = e.currentTarget.value;
				var newMessage = whatsappURL;
				if (message != "") {
					newMessage += '&text=';
					newMessage += message;
					newMessage.replace("\n", "<br/>");
				...
				whatsappLink.setAttribute('href', newMessage);
			})
			...
			if (whatsapp) { whatsapp.parentNode.removeChild(whatsapp) };
	...
	build = function(){
		...
		console.log("%cCreated by Hussain Ali","color: #000; padding: 5px 0px;"),console.log("I LOVE CODING");
